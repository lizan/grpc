...
static alts_shared_resource* resource = alts_get_shared_resource();
...
static void thread_worker(void* arg) {
  while (true) {
    grpc_event event = grpc_completion_queue_next(
        resource->cq, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr);
...
static void init_shared_resources(const char* handshaker_service_url) {
  ...
  if (resource->channel == nullptr) {
    gpr_cv_init(&resource->cv);
    resource->channel =
        grpc_insecure_channel_create(handshaker_service_url, nullptr, nullptr);
    resource->cq = grpc_completion_queue_create_for_next(nullptr);
    gpr_thd_options options = gpr_thd_options_default();
    gpr_thd_options_set_joinable(&options);
    gpr_thd_new(&resource->thread_id, "alts_tsi_handshaker", &thread_worker, nullptr,
                &options);
  }
...
tsi_result alts_tsi_handshaker_create(
  ...
  alts_handshaker_client* client = alts_grpc_handshaker_client_create(
      resource->channel, resource->cq, handshaker_service_url);
  if (client == nullptr) {
