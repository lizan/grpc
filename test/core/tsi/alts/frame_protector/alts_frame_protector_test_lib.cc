...
static void alts_test_send_bytes_to_peer(bool is_client, unsigned char* buf,
  ...
  /* Log sending info. */
  const char* sender = is_client ? "Client" : "Server";
  gpr_log(GPR_INFO, "%s sending %zu bytes to peer.", sender, buf_size);
...
static void alts_test_receive_bytes_to_peer(bool is_client, unsigned char* buf,
  ...
  /* Log receiving info. */
  const char* receiver = is_client ? "Client" : "Server";
  gpr_log(GPR_INFO, "%s receiving %zu bytes from peer.", receiver, to_read);
  ...
  while (!done && result == TSI_OK) {
    ...
    do {
      ...
      if (message_buffer_size > 0) {
        gpr_log(GPR_INFO, "Wrote %zu bytes to message.", message_buffer_size);
