...
static bool grpc_alts_rpc_protocol_versions_equal(
    grpc_alts_rpc_protocol_versions* l_versions,
    grpc_alts_rpc_protocol_versions* r_versions) {
...
static void test_success() {
  grpc_alts_rpc_protocol_versions version;
  grpc_alts_rpc_protocol_versions decoded_version;
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_max(
      &version, kMaxRpcVersionMajor, kMaxRpcVersionMinor));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_min(
      &version, kMinRpcVersionMajor, kMinRpcVersionMinor));
  /* Serializes to raw bytes. */
  size_t encoded_length =
      grpc_alts_rpc_protocol_versions_encode_length(&version);
  uint8_t* encoded_bytes = static_cast<uint8_t*>(gpr_malloc(encoded_length));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_encode_to_raw_bytes(
      &version, encoded_bytes, encoded_length));
  grpc_slice encoded_slice;
  /* Serializes to grpc slice. */
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_encode(&version,
                                                             &encoded_slice));
  ...
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_decode(encoded_slice,
                                                             &decoded_version));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_equal(&version,
                                                            &decoded_version));
...
static void test_failure() {
  grpc_alts_rpc_protocol_versions version, decoded_version;
  grpc_slice encoded_slice;
  ...
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_encode_length(nullptr) ==
             0);
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_max(
      &version, kMaxRpcVersionMajor, kMaxRpcVersionMinor));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_min(
  ...
  GPR_ASSERT(!grpc_alts_rpc_protocol_versions_encode(nullptr,
                                                              &encoded_slice));
  GPR_ASSERT(
      !grpc_alts_rpc_protocol_versions_encode(&version, nullptr));
  GPR_ASSERT(
      !grpc_alts_rpc_protocol_versions_decode(encoded_slice, nullptr));
  ...
  grpc_slice_unref(encoded_slice);
  GPR_ASSERT(!grpc_alts_rpc_protocol_versions_decode(
      bad_slice, &decoded_version));
  grpc_slice_unref(bad_slice);
  gpr_free(encoded_bytes);
}

static void test_copy() {
  grpc_alts_rpc_protocol_versions src;
  grpc_alts_rpc_protocol_versions des;
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_max(
      &src, kMaxRpcVersionMajor, kMaxRpcVersionMinor));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_min(
      &src, kMinRpcVersionMajor, kMinRpcVersionMinor));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_copy(&src, &des));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_equal(&src, &des));
}

static void test_check_success() {
  grpc_alts_rpc_protocol_versions v1;
  grpc_alts_rpc_protocol_versions v2;
  ...
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_set_min(
      &v2, kMaxRpcVersionMajor, kMaxRpcVersionMinor));
  GPR_ASSERT(grpc_alts_rpc_protocol_versions_check(
                 (const grpc_alts_rpc_protocol_versions*)&v1,
                 (const grpc_alts_rpc_protocol_versions*)&v2,
                 &highest_common_version) == 1);
  GPR_ASSERT(grpc_alts_rpc_protocol_version_compare(
