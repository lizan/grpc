...
static void test_success() {
  ...
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_encode(&version,
                                                             &encoded_slice));
  ...
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_decode(encoded_slice,
                                                             &decoded_version));
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_equal(&version,
                                                            &decoded_version));
...
static void test_failure() {
  ...
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_encode_length(nullptr) ==
             0);
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_set_max(
      &version, kMaxRpcVersionMajor, kMaxRpcVersionMinor));
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_set_min(
  ...
  GPR_ASSERT(!grpc_gcp_rpc_protocol_versions_encode(nullptr,
                                                              &encoded_slice));
  GPR_ASSERT(
      !grpc_gcp_rpc_protocol_versions_encode(&version, nullptr));
  GPR_ASSERT(
  ...
  grpc_slice_unref(encoded_slice);
  GPR_ASSERT(!grpc_gcp_rpc_protocol_versions_decode(
...
static void test_check_success() {
  ...
  GPR_ASSERT(grpc_gcp_rpc_protocol_version_compare(
